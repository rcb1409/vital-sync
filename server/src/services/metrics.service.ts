...
async function logHabits(input: LogHabitsInput) {
  ...
  const result = await prisma.dailyHabit.upsert({
    where: {
      userId_date: {
        userId: input.userId,
        date: parsedDate,
  ...
  // Granular Invalidation: only destroy habit blocks, streak blocks, and AI context cache
  await cacheService.invalidateSpecific(input.userId, ['habit', 'streaks', 'ctx']);
  return result;
}

async function getHabitForDate(userId: string, dateString: string) {
  return cacheService.cacheAside(`metrics:${userId}:habit:${dateString}`, 300, async () => {
...
async function getHabitsHistory(userId: string, rangeDays: number) {
  return cacheService.cacheAside(`metrics:${userId}:habits:${rangeDays}`, 300, async () => {
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - rangeDays);
...
async function getStreaks(userId: string) {
  return cacheService.cacheAside(`metrics:${userId}:streaks`, 300, async () => {
